...
const loadData = async () => {
    try {
        const url = "/api/unit-of-measurement";
...
const submit = async () => {
    try {
        const url = isEdit ? "/api/unit-of-measurement/update" : "/api/unit-of-measurement/create";
...
const deleteData = async (code) => {
    try {
        const url = `/api/unit-of-measurement/delete`;
...
$("#mainTable").on("click", `button[action="edit"]`, function () {
    ...
    isEdit = true;
    const { code, name, description, remark } = data;
    formTab.show();
